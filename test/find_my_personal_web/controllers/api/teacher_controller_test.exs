defmodule FindMyPersonalWeb.Api.TeacherControllerTest do
  ...
  describe "index" do
    test "lists all teacher", %{conn: conn} do
      conn = get(conn, Routes.api_teacher_path(conn, :index))
  ...
  describe "create teacher" do
    test "renders teacher when data is valid", %{conn: conn} do
      conn = post(conn, Routes.api_teacher_path(conn, :create), teacher: @create_attrs)
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      conn = post(conn, Routes.api_teacher_path(conn, :create), teacher: @invalid_attrs)
  ...
  describe "update teacher" do
    ...
    test "renders teacher when data is valid", %{conn: conn, teacher: %Teacher{id: id} = teacher} do
      conn = put(conn, Routes.api_teacher_path(conn, :update, teacher), teacher: @update_attrs)
  ...
  describe "delete teacher" do
    ...
    test "deletes chosen teacher", %{conn: conn, teacher: teacher} do
      conn = delete(conn, Routes.api_teacher_path(conn, :delete, teacher))
      assert response(conn, 204)

      assert_error_sent 404, fn ->
        get(conn, Routes.api_teacher_path(conn, :show, teacher))
      end
    end
  end
